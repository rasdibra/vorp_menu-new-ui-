(function () {
    ...
    function rdMouseSelect(namespace, name, menuElement, index, selector) {
        const menuData = MenuData.opened[namespace][name];
        if (!menuData || !menuData.elements || !menuData.elements[index]) return;
        if (MenuData.pos[namespace][name] === index) return;
        ...
        $menu.find('.menu-item, .grid-item').removeClass('selected');
        $menu.find(selector).eq(index).addClass('selected');
    ...
    function addMouseEvents(namespace, name, menuElement) {
        ...
        $(menuElement).find('.menu-item').each(function (index) {
            ...
            $(this).on('mouseenter', function () {
                if (isDragging || isDescDragging || isMultiSliderDragging) return;
                rdMouseSelect(namespace, name, menuElement, index, '.menu-item');
            });
        ...
        $(menuElement).find('.grid-item').each(function (index) {
            ...
            $(this).on('mouseenter', function () {
                if (isDragging || isDescDragging || isMultiSliderDragging) return;
                rdMouseSelect(namespace, name, menuElement, index, '.grid-item');
            });
        });
    ...
    MenuData.open = function (namespace, name, data) {
        // RD update: mouse cursor enabled by default so all VORP menus are clickable.
        // Resources can still pass enableCursor = false if they need a keyboard-only menu.
        if (typeof data.enableCursor === "undefined") {
            data.enableCursor = true;
        }
